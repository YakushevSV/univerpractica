...
public class ServerHandler implements HttpHandler {
    ...
    private static final Logger REQUESTLOG = new FileLogger("serverlog.txt");
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        REQUESTLOG.info("request end");
    }

    private Response dispatch(HttpExchange httpExchange) {
        REQUESTLOG.info("method " + httpExchange.getRequestMethod());
    ...
    private Response doGet(HttpExchange httpExchange) {
        ...
        if (query == null) {
            REQUESTLOG.info("response: absent query in request");
            return Response.badRequest("Absent query in request");
        }
        Map<String, String> map = queryToMap(query);
        String token = map.get(Constants.REQUEST_PARAM_TOKEN);
        REQUESTLOG.info("request parameters: token=" + token);
        if (StringUtils.isEmpty(token)) {
            REQUESTLOG.info("response: token parameter is required");
        ...
        try {
            ...
            if (index > messageStorage.size()) {
                REQUESTLOG.info("response: incorrect token in request");
            ...
            REQUESTLOG.info("response: history size=" + messageStorage.size());
            return Response.ok(responseBody);
        } catch (InvalidTokenException e) {
            REQUESTLOG.info("response: incorrect format of token");
    ...
    private Response doPost(HttpExchange httpExchange) {
        try {
            ...
            REQUESTLOG.info("request parameter: message=" + message);
            logger.info(String.format("Received new message from user: %s", message));
            messageStorage.add(message);
            return Response.ok();
        } catch (ParseException e) {
            logger.error("Could not parse message.", e);
            REQUESTLOG.info("response: incorrect request body");
    ...
    private void sendResponse(HttpExchange httpExchange, Response response) {
        try (OutputStream os = httpExchange.getResponseBody()) {
            ...
            REQUESTLOG.info("could not send response");
